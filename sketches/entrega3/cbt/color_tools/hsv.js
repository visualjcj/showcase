...
function preload() {
  hsvShader = readShader('/showcase/sketches/entrega3/cbt/color_tools/hsv.frag', { varyings: Tree.texcoords2 });
  // image source: https://en.wikipedia.org/wiki/HSL_and_HSV#/media/File:Fire_breathing_2_Luc_Viatour.jpg
  img = loadImage('/showcase/sketches/entrega3/cbt/color_tools/fire_breathing.png');
}
...
function setup() {
  ...
  colorTexture.changed(() => { hsvShader.setUniform('colorTexture', colorTexture.value()) })
  ...
  hsvSelect.changed(() => { hsvShader.setUniform('selectedTool', hsvSelect.value()) })
